...
(defun jest-file (file &optional args)
  "Miha Run jest on FILE, using ARGS.
...
With a prefix argument, allow editing."
  (interactive
   (list
    (buffer-file-name)
    (jest-arguments)))
  (message (jest--sensible-test-file file))
...
When pointer is not inside a test function jest is run on the whole file."
  (interactive
   (list (buffer-file-name) (jest-test-unit-at-point) (jest-arguments)))
  (jest--run
   :args args
   :file file
   :testname testname))
...
(cl-defun jest--run-command (&key command popup-arguments edit)
  ...
  (let* ((default-directory (jest--project-root)))
    (message (concat "HHHH " default-directory))
    (when jest-confirm
      (setq edit (not edit)))
    (when edit
      (setq command
...
(cl-defun jest--run-as-comint (&key command popup-arguments)
  ...
  (let* ((buffer (jest--get-buffer))
    ...
    (with-current-buffer buffer
      ...
      (make-comint-in-buffer "jest" buffer "powershell" nil "-c" command)
...
(defun jest--transform-arguments (args)
  ...
  (-->
   args
   (jest--switch-to-option it "--color" "--color=no" "--color=yes")
